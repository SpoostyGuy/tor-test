...
class CreatedCell {
    ...
    parse(mainBody) {
        ...
        if (utils.h_auth(auth_input).toString('hex') == AUTH.toString('hex')) {
            ...
            var backDecipherMain = crypto.createDecipheriv('aes-128-ctr', keyData.slice(56, 72), Buffer.alloc(16))
            backDecipherMain.setAutoPadding(false)
            var backDecipherOther = crypto.createDecipheriv('aes-128-ctr', keyData.slice(56, 72), Buffer.alloc(16))
            backDecipherOther.setAutoPadding(false)
            ...
            return {
                ...
                backwardKey: {
                    main: backDecipherMain,
                    before: backDecipherOther
                },
...
class Cell {
    ...
    recieveCells() {
        var isContinue = undefined
        var otherContinue = undefined
        var otherCommand = 0
        this.circuit.socket.on('data', (buf) => {
            var totalParsedCircuitData = {}
            var offset = 0

            if (isContinue != undefined) {
                console.log("WORK CONTINUE")
                var payload = buf.slice(offset, offset+(509-isContinue.length))
                payload = Buffer.concat([isContinue, payload])
                offset += (509-isContinue.length)
                if (totalParsedCircuitData[otherCommand] != undefined) {
                    if (!Array.isArray(totalParsedCircuitData[otherCommand])) {
                        totalParsedCircuitData[otherCommand] = [totalParsedCircuitData[otherCommand]]
                    }
                    totalParsedCircuitData[otherCommand].push(this.parseCell(payload, otherCommand))
                } else {
                    totalParsedCircuitData[otherCommand] = this.parseCell(payload, otherCommand)
                }
                isContinue = undefined
                otherCommand = 0
            }
            if (otherContinue != undefined) {
                buf = Buffer.concat([otherContinue, buf])
                console.log('continuing')
                otherContinue = undefined
            }
            var offsetBefore = 0
            try {
                if ((buf.length-offset) > 0) {
                    while (true) {
                        if ((buf.length-offset) < 11) {
                            otherContinue = buf.slice(offsetBefore)
                            console.log(otherContinue.length)
                        }
                        var thisLen = 0
                        var curcuitId = 0
                ...
                console.log('looks like we got an error')
                otherContinue = buf.slice(offsetBefore)
                console.log(otherContinue.length)
                console.log(e)
            }
            this.circuit.onCells(totalParsedCircuitData)
        })
    }
