...
class Circuit {
    ...
    async retrieveCerts(isExit) {
        while (true) {
            var hostData = await relayDB.retrieveRelay(isExit, this.logger, this.socketList, this.useLocation)
            ...
            this.socketList.push(hostData.ip)
    ...
    async handleCallback(callback, streamId) {
        await request.handleCallback(callback, streamId)
    }
    async endRelayStream(streamId) {
        await request.endRelayStream(this, streamId)
    }

    async speedTest() {
        var name = Buffer.concat([Buffer.from("www.google.com"), Buffer.from([0])])
        var resData = new RelayCell(this).buildBody({
            RelayCommand: 11,
            StreamID: 1,
            Body: name
        })
        resData = new types.Cell().buildCell(3, resData, this.circuitId, true)
        var timeDelay = Date.now()
        await this.writeAndWaitForResponse(
            resData
        )
        timeDelay = (Date.now()-timeDelay)
        var avgDistance = 0
        this.socketList.forEach((ip, index) => {
            if (index > 0) {
                var beforeGeo = geoIP.allData(this.socketList[index-1])
                var thisGeo = geoIP.allData(ip)
                avgDistance += relayDB.getDistanceFromLatLonInKm(beforeGeo.location, thisGeo.location)
            }
        })
        return {
    ...
    async connect(useLocation, startTime, attempts) {
        this.time = new Date()
        this.useLocation = useLocation
        try {
            this.logger.connectionMessage("Finding gaurd node...")
            var hostData = await relayDB.retrieveRelay(false, this.logger, this.socketList, this.useLocation)
            ...
            this.socket.on('ready', async () => {
                ...
                var buf = this.cellHandler.buildCell(7, new types.VersionCell([3,4]).build(), 0, false)
                ...
                if (parsedData['129'] == undefined) {
                    ...
                    request.handleTotalCallbacks(this)
                    this.cellHandler.recieveCells()
            ...
            if (this.failedVal == false) {
                this.logger.connectionMessage("Running speed test...")
                var speed = await this.speedTest()
                await this.logger.finished()
                
                console.log('\x1b[1;32m✓\x1b[0m Built circuit; \x1b[38;5;8m' + this.socketList.join(', ') + '\x1b[0m')
                console.log('⧗ ' + speed.ping + 'ms of delay, ' + Math.floor(speed.distance) + 'km distance, ' + (Date.now()-startTime)/1000 + 's build time, ' + attempts + ' attempt count')
                return true
            } else {
                await this.logger.finished()
                return false
